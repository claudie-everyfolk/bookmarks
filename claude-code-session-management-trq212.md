...
**Saved:** 2026-04-16
**Tags:** claude-code, session-management, context-window, training-material

---

## Why this matters

Authoritative guide from an Anthropic engineer on session management best practices for Claude Code. Directly relevant to our Claude Code training decks — covers the exact topics clients ask about.
...
### Context Rot
- Performance degrades as context grows (~300-400k tokens for 1M model, task-dependent)
- Attention gets spread across more tokens; older content distracts from current task

### Every Turn Is a Branching Point
After Claude finishes a turn, you have 5 options:
1. **Continue** — send another message in the same session
2. **/rewind** (esc esc) — jump back to a previous message and retry from there
3. **/clear** — start a new session with a distilled brief
4. **Compact** — summarize the session so far and keep going
5. **Subagents** — delegate to an agent with its own clean context

### When to Start a New Session
- **Rule of thumb:** new task = new session
- **Grey area:** related tasks where some context is still useful (e.g., writing docs for a feature you just implemented — efficiency gain outweighs context noise)

### Rewinding Instead of Correcting
- Double-tap Esc or /rewind to jump back to any previous message
- Better than typing "that didn't work, try X" — drops failed approach from context entirely
- Can use "summarize from here" to create a handoff message before rewinding

### Compacting vs. Fresh Sessions
- **/compact** — model summarizes conversation, replaces history with summary. Lossy but automatic. Can steer with instructions: `/compact focus on the auth refactor, drop the test debugging`
- **/clear** — you write the brief manually. More work, but the context is exactly what you decided was relevant

### What Causes Bad Compacts
- Model can't predict your next direction
- Example: autocompact fires after debugging, summarizes the investigation, but your next message is about a different warning that got dropped
- Context rot means the model is at its *least* intelligent when compacting
- With 1M context, you have more time to `/compact` proactively with a description

### Subagents as Context Management
- Subagents get their own fresh context window
- Useful when intermediate output won't be needed again — only the conclusion comes back
- Mental test: "Will I need this tool output again, or just the conclusion?"
- Explicit prompts:
  - "Spin up a subagent to verify the result based on this spec"
  - "Spin off a subagent to read this other codebase and summarize how it implemented auth"
...
In summary, when Claude has ended a turn and you're about to send a new message, you have a decision point. Overtime we expect that Claude will help you handle this itself, but for now this is one of the ways you can guide Claude's output.
