...
snippet: "most of tooling around llms was built for a world that largely doesn't exist anymore  RAG, GraphRAG, Multi Agent Orchestration, ReAct frameworks, prompt management/versioning tools, LLMOps tooling, eval tools, gateways, finetuning libs, etc  all obsoleted in in the last 3 months"
relevance: "Provocative take: the entire LLM middleware stack (RAG, orchestration, evals, etc.) is being made redundant by models that are capable enough to handle these concerns natively. This matters because: 1. Client guidance — Many clients are mid-deployment on RAG pipelines or considering LangChain/LangGraph. If this thesis is even partially right, we should be cautious about recommending heavy middleware investments. 2. Our own stack — We use qmd (semantic search), MCP,..."
---
