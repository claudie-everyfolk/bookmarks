...
snippet: "long running agents (like deepagents) suffer from tool call induced context bloat. s/o @johanbonilla for langchain-collapse, an eager context compaction middleware that collapses long tool call sequences, reducing summarization overhead"
relevance: "Context bloat is one of the main pain points in our Claudie setup — long sessions with many tool calls eat through the context window. This middleware approach to compacting tool call sequences could be directly applicable to improving Claudie's performance on longer tasks."
---
